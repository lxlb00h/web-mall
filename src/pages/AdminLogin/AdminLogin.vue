        ...
        <form>
          <div class="current">
            <section class="login-select">
              <el-select aria-placeholder="请选择身份" @change="onSelectedDrug($event)" v-model="id" style="width: 100%;height: 100%;border-radius: 4px;" value-key="item.id">
                <el-option v-for="item in kindList" :key="item.id" :label="item.name" :value="item"></el-option>
              </el-select>
            </section>
  ...
  export default {
    ...
    data() {
      return {
        ...
        typeId: '',//管理员类型
        adminInfo: {},
        kindList:[{name:"商品专员",id:1},{name:"营销经理",id:2},{name:"系统管理员",id:3}]
      }
    },
    methods: {
      onSelectedDrug(e){
      let obj = {};
      obj = this.kindList.find((item)=>{//这里的userList就是上面遍历的数据源
      ...
      async login() {
        ...
        if(result.message === "登录成功！"){
          ...
          if(this.typeId==1){
              ...
              this.$router.replace('/marketmanager');
